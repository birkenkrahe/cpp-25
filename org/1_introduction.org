...
* Which data structures do you already know?
...
- These are *not* data structures:
  #+begin_quote
  1. Data types (~int~, ~double~, ~char~, ~string~, ~bool~)
  2. Variables (=int x = 5=)
  3. Constants (=const float PI = 3.14=)
  4. Operators (~+=~, ~++~, ~==~ etc.)
  5. Control flow (~if~, ~else if~, ~else~, ~switch~)
  6. Functions (=int f(int x) { return x*x; }=)
  7. Pointers (=int *p = &i;=)
  8. References (=int & ref = x;=)
  9. Exceptions (~try~, ~catch~, ~throw~)
  10. Preprocessor directives (~#include~, ~#define~, ~#ifndef~)
  #+end_quote

- What about media like videos, images, graphs etc.? Are these data,
...
- Some data structures are designed to allow efficient execution of
  certain algorithms, e.g. *hash tables* for key-based search (maps).
...
- *C++ array vs. vector:*
  ...
  1) *Arrays* are *static* collections of elements of the same type,
     stored in contiguous memory locations. They are used to store
     multiple values in one variable. Values can be accessed using an
  ...
  2) *Vectors* are *dynamic* arrays provided by the C++ Standard Template
     ...
     useful member functions, like ~.size()~.
...
- If C++ is not (yet) your mojo, think of the difference between
  *lists*, *tuples*, and *dictionaries* in Python:
  ...
  2) *list* elements are ordered (indexed), but *dictionaries* are
     unordered.
...
- Avoid Maslow's hammer ("If your toolbelt only has a hammer, you will
  ...
  tighten a screw? The answer is *abstraction*.

- But never too much: Abstraction is great (C), too much abstraction
  makes things harder than they need to be at the start (C++).
...
#+begin_src C++ :noeval :tangle hello.cpp :main yes :includes <iostream> :results output :exports both
  std::string name;
...
1) Input from the keyboard, from a file, or from a URL (network)
2) Output to the screen, to a file, or to a server (network)
3) Output with ~cout~ or ~printf~ (~iostream~)
4) User name stored as ~string~ or as ~char~ array or a ~vector~
5) User name retrieved with ~cin~ (~>>~) or ~getline~ or ~scanf~
6) Concatenate greeting message with =+= or stream with =<<=
7) With a user-defined function or as standalone command
8) With a user-defined ~class~ or ~struct~ or ~union~
