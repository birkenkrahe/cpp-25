...
- What does designing a =BankAccount= ~class~ mean?
  ...
  4) use the objects exclusively through ~public~ member functions like
     =getBalance=, =depositAmount=.
  #+end_quote

- What's an *object*?
  #+begin_quote
  An object is a resource used to complete the task - no difference
  to other resources (variables) - region in memory.

  To the ~class~ the object is an instance of what the class
  represents - like a =lamp= that is a =Light= object.
  #+end_quote

- Example of an object: A savings account, and we have a
  =BankAccount=. What does it have?
  #+begin_quote
  A savings account is a special type of =BankAccount=. Just like 5 is
  a special integer (~int~).
...
- What's a constructor? Example?
  #+begin_quote
  A constructor constructs an object of the chose class.
  In =BankAccount=, our constructor creates an account with a
  starting balance.
  #+end_quote
...
- Example: Code along using OneCompiler.Com. Insert this code between
  ...
  #+name: BankAccount
  #+begin_src C++ :main no :includes <iostream> <cstdlib> <string> <fstream> <vector> :namespaces std :results output
    class BankAccount
    {
    private:
...
- The main differences to the *inline* definition:
  ...
  2) The function needs to be brought into scope with the prefix
     =BankAccount::=
  3) The function definition must be located after the ~class~
     declaration, and before the ~main~ function.
  4) Typically, declarations are put into header files ~.h~, while the
     member functions are in a separate file ~.cpp~. The header is given
     to the preprocessor, and the definitions file is compiled
     separately (it becomes an ~.o~ object file) and later linked to the
     object file for the ~main~ program.
  #+end_quote

- What's going on conceptually here?
  #+begin_quote
  This is an example of separating interface and implementation - it
  mirrors the separation of function prototype and function definition
...
- What is a constructor?
  #+begin_quote
  A constructor is a ~public~ member function that is automatically
  called when a ~class~ is created: =Rectangle rect;=
...
- What if a constructor has default arguments for all its parameters?
  #+begin_quote
  Then it can be called without explicit arguments - it becomes the
  default constructor.
  #+end_quote
...
- What if all constructors of a ~class~ require arguments?
  #+begin_quote
  Then there is no default constructor. Now if you don't create
  with passing arguments, you get an error.
  #+end_quote
...
- What is a destructor?
  ...
  when an object is destroyed (i.e. its memory is returned).
  #+end_quote

- When is an object destroyed?
  #+begin_quote
  When it goes out of scope. Examples: at the end of a function if the
  objects are all local, or when they are deleted with ~delete~ after
  having been created with ~new~.
  #+end_quote
